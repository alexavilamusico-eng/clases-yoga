...
(function () {
  ...
  /* ---------- armar clase automáticamente ----------
     Basado en cómo se estructura una clase de yoga profesional (método de
     postura cumbre / "peak pose" — Mark Stephens, "Yoga Sequencing"; principios
     de secuenciación de Yoga Journal; Yin de Paul Grilley; Restaurativo de
     Judith Hanson Lasater):
       - arco de montaña: llegada → calentamiento → desarrollo con UNA postura
         cumbre (~65-70% de la clase) → enfriamiento con contrapostura → Savasana.
       - la cumbre se elige primero, no al final: las demás posturas de Desarrollo
         se escogen para PREPARARLA (misma zona/dinámica — p. ej. si la cumbre abre
         cadera, antes hay más aperturas de cadera), y Enfriamiento busca la misma
         zona con la dinámica contraria (contrapostura real, no solo "algo suave").
       - dentro de cada sección, las posturas se agrupan primero por familia
         (de pie / sentada / supina…) y luego por zona del cuerpo, para no saltar
         de una parte a otra sin relación.
       - Calentamiento abre en el suelo/sentado y cierra más activo, preparando el
         Desarrollo que sigue; Enfriamiento hace lo inverso: empieza más activo y
         termina en el piso, justo antes de Savasana.
     El estilo cambia el ritmo: cuántas posturas, cuánto se sostienen y qué
     familias predominan. La semilla (tema / zona / dinámica) sesga la elección. */
  var CLAMP = function (x, a, b) { return Math.max(a, Math.min(b, x)); };
  function nivelRank(n) { return n === "avanzado" ? 3 : n === "intermedio" ? 2 : 1; }
  // familias de "activa/de pie" a "pasiva/en el piso" — para ordenar dentro de la sección
  var RAMPA = ["de pie", "equilibrio", "equilibrio de brazos", "invertida", "arrodillada", "prona", "sentada", "supina", "restaurativa"];
  function rangoFamilia(p) {
    for (var i = 0; i < RAMPA.length; i++) if (p.tipo.indexOf(RAMPA[i]) > -1) return i;
    return RAMPA.length;
  }
  // zonas del cuerpo en el orden en que se suelen calentar/soltar (de lo distal a lo central):
  // así dos posturas de la misma zona quedan juntas en vez de alternar sin lógica.
  var ZONA_ORDEN = ["tobillos y pies", "cuádriceps", "isquiotibiales", "ingles", "caderas", "glúteos",
    "psoas", "zona lumbar", "columna", "core", "pecho", "hombros", "cuello", "muñecas"];
  function zonaRango(p) {
    var best = ZONA_ORDEN.length;
    (p.zona || []).forEach(function (z) { var i = ZONA_ORDEN.indexOf(z); if (i > -1 && i < best) best = i; });
    return best;
  }
  function esBilateral(p) {
    return p.dinamica.indexOf("torsión") > -1 || p.dinamica.indexOf("flexión lateral") > -1
      || (p.tipo.indexOf("de pie") > -1 && p.dinamica.indexOf("equilibrio") > -1);
  }

  // Plantillas de estilo. Cada sección:
  //   w        peso de tiempo (Savasana, Desarrollo, etc.)
  //   maxN     tope de posturas de esa sección
  //   nivelMax tope de dificultad
  //   semilla  cuánto pesa la semilla aquí (fuerte solo en Desarrollo)
  //   famMas/famMenos    familias `tipo` que suben / bajan el puntaje
  //   dinMas/dinMenos    dinámicas que suben / bajan
  //   peak     true = insertar postura cumbre hacia el 66%
  //   texto    true = abrir con una nota de respiración/intención
  // El hold de cada postura se calcula para llenar su sección (minutos), acotado
  // al rango típico del estilo — así "~X / Y min" cuadra. Andrea puede cambiar
  // cualquiera a respiraciones con el toggle de siempre.
  var RANGO = { fluido: [1, 2.5], medio: [1.5, 3.5], largo: [2, 4], yin: [3, 5], resto: [5, 12] };

  function base(rango, minPP, maxN, nivelDes) {
    return {
      minPP: minPP, maxN: maxN, rango: rango,
      secs: [
        { t: "Llegada · centramiento", w: 1, texto: 1, semilla: 0.5, nivelMax: "principiante", maxN: 3,
          famMas: ["sentada", "supina", "arrodillada"], famMenos: ["de pie", "equilibrio", "equilibrio de brazos", "invertida", "prona"],
          dinMas: ["quietud", "apertura de cadera"], dinMenos: ["fuerza", "inversión", "extensión de columna"] },
        { t: "Calentamiento", w: 2, semilla: 1, nivelMax: "principiante",
          famMas: ["arrodillada", "sentada", "de pie"], famMenos: ["equilibrio de brazos", "invertida"],
          dinMas: ["extensión de columna", "apertura de cadera", "flexión hacia adelante", "torsión"], dinMenos: ["fuerza"] },
        { t: "Desarrollo", w: 4, peak: true, semilla: 4, nivelMax: nivelDes,
          famMas: ["de pie", "equilibrio", "invertida", "equilibrio de brazos"], famMenos: ["restaurativa"],
          dinMas: ["fuerza", "equilibrio", "extensión de columna", "flexión lateral"], dinMenos: ["quietud"] },
        { t: "Enfriamiento", w: 2, semilla: 1, nivelMax: "principiante", maxN: 6,
          famMas: ["sentada", "supina", "restaurativa"], famMenos: ["de pie", "equilibrio de brazos", "invertida", "prona"],
          dinMas: ["flexión hacia adelante", "torsión", "apertura de cadera", "quietud"], dinMenos: ["fuerza", "extensión de columna"] },
        { t: CIERRE, w: 1, cierre: true },
      ],
    };
  }
  var TEMPLATES = {
    "vinyasa": base(RANGO.fluido, 1.7, 11, "intermedio"),
    "hatha": base(RANGO.largo, 2.9, 7, "intermedio"),
    "hatha-vinyasa": base(RANGO.medio, 2.2, 9, "intermedio"),
    "ashtanga": base(RANGO.fluido, 1.6, 12, "avanzado"),
    "suave / terapéutico": (function () {
      var t = base(RANGO.medio, 2.6, 7, "principiante");
      t.secs[2].peak = false; t.secs[2].famMas = ["de pie", "sentada"]; t.secs[2].dinMenos = ["fuerza", "inversión"];
      return t;
    })(),
    "yin": {
      minPP: 3.5, maxN: 6, rango: RANGO.yin,
      secs: [
        { t: "Llegada · respiración", w: 1, texto: 1, semilla: 0.5, nivelMax: "principiante", maxN: 2,
          famMas: ["sentada", "supina"], famMenos: ["de pie", "equilibrio de brazos", "invertida", "prona"], dinMas: ["quietud"], dinMenos: ["fuerza"] },
        { t: "Cuerpo yin", w: 4, semilla: 2, nivelMax: "intermedio",
          famMas: ["sentada", "supina", "prona"], famMenos: ["de pie", "equilibrio", "equilibrio de brazos", "invertida"],
          dinMas: ["flexión hacia adelante", "apertura de cadera", "flexión lateral", "extensión de columna"], dinMenos: ["fuerza", "equilibrio"] },
        { t: "Enfriamiento yin", w: 2, semilla: 1, nivelMax: "principiante", maxN: 4,
          famMas: ["supina", "sentada"], famMenos: ["de pie", "prona", "equilibrio de brazos", "invertida"], dinMas: ["torsión", "apertura de cadera", "quietud"], dinMenos: ["fuerza"] },
        { t: CIERRE, w: 2, cierre: true },
      ],
    },
    "restaurativo": {
      minPP: 8, maxN: 4, rango: RANGO.resto,
      secs: [
        { t: "Llegada", w: 1, texto: 1, semilla: 0.5, nivelMax: "principiante", maxN: 1,
          famMas: ["sentada", "supina"], famMenos: ["de pie", "equilibrio de brazos", "invertida", "prona"], dinMas: ["quietud"], dinMenos: ["fuerza"] },
        { t: "Apoyos", w: 5, semilla: 1.5, nivelMax: "principiante",
          famMas: ["restaurativa", "supina", "sentada"], famMenos: ["de pie", "equilibrio", "equilibrio de brazos", "invertida", "prona"],
          dinMas: ["quietud", "apertura de cadera", "flexión hacia adelante"], dinMenos: ["fuerza", "equilibrio", "inversión"] },
        { t: CIERRE, w: 2, cierre: true },
      ],
    },
  };

  // foco (opcional): { zona, dinamica } de la postura cumbre de la clase. Sirve para dos cosas
  // según la sección — en Desarrollo prepara esa zona/dinámica (calienta lo que la cumbre va a
  // pedir); en Enfriamiento solo pasa la zona (sin dinámica) para buscar el contrapeso: misma
  // zona, pero la sección ya pide la dinámica opuesta (flexión/torsión/quietud) por su cuenta.
  function elegirPosturas(sem, s, n, usados, foco) {
    var maxR = nivelRank(s.nivelMax || "avanzado");
    var scored = POSES.filter(function (p) {
      return !usados[p.slug] && p.slug !== "savasana" && nivelRank(p.nivel) <= maxR;
    }).map(function (p) {
      var sc = Math.random() * 0.5;
      if (semillaMatch(p, sem)) sc += s.semilla != null ? s.semilla : 2;
      (s.famMas || []).forEach(function (t) { if (p.tipo.indexOf(t) > -1) sc += 1.6; });
      (s.famMenos || []).forEach(function (t) { if (p.tipo.indexOf(t) > -1) sc -= 3; });
      (s.dinMas || []).forEach(function (d) { if (p.dinamica.indexOf(d) > -1) sc += 1.1; });
      (s.dinMenos || []).forEach(function (d) { if (p.dinamica.indexOf(d) > -1) sc -= 1.6; });
      if (s.nivelMax === "principiante" && p.nivel !== "principiante") sc -= 1.4;
      if (foco) {
        if ((p.zona || []).some(function (z) { return foco.zona.indexOf(z) > -1; })) sc += 1.3;
        if ((p.dinamica || []).some(function (d) { return foco.dinamica.indexOf(d) > -1; })) sc += 0.9;
      }
      return { p: p, sc: sc };
    }).filter(function (x) { return x.sc > 0; }).sort(function (a, b) { return b.sc - a.sc; });
    return scored.slice(0, Math.max(1, n)).map(function (x) { return x.p; });
  }

  function elegirCumbre(sem, usados, maxRank) {
    var scored = POSES.filter(function (p) {
      return !usados[p.slug] && p.slug !== "savasana" && nivelRank(p.nivel) <= maxRank + 1 && nivelRank(p.nivel) >= 2;
    }).map(function (p) {
      var sc = Math.random() * 0.4;
      if (semillaMatch(p, sem)) sc += 5;
      if (p.dinamica.indexOf("extensión de columna") > -1 || p.dinamica.indexOf("fuerza") > -1 || p.dinamica.indexOf("equilibrio") > -1 || p.dinamica.indexOf("inversión") > -1) sc += 1.5;
      sc += nivelRank(p.nivel) * 0.2;
      return { p: p, sc: sc };
    }).sort(function (a, b) { return b.sc - a.sc; });
    return scored.length ? scored[0].p : null;
  }

  function armarClaseAuto(c) {
    var estilo = (c.estilo || "").toLowerCase().trim();
    var tpl = TEMPLATES[estilo] || TEMPLATES["vinyasa"];
    var total = +c.objetivo || 60;
    var rango = tpl.rango || RANGO.fluido;
    var sumaW = tpl.secs.reduce(function (a, s) { return a + s.w; }, 0);
    var usados = {};
    var cumbre = null; // la postura cumbre, para que Desarrollo la prepare y Enfriamiento la contrarreste
    repartoManual = {};

    c.bloques = tpl.secs.map(function (s) {
      var mins = Math.max(3, Math.round(total * s.w / sumaW));
      var bl = { id: uid("b"), titulo: s.t, objetivo: mins, peso: s.w, items: [] };

      if (s.cierre) {
        if (bySlug["savasana"]) bl.items.push({ tipo: "postura", slug: "savasana", lado: "", dur: CLAMP(mins, 5, 15), durUnit: "min", nota: "" });
        return bl;
      }
      var libres = mins;
      if (s.texto) { bl.items.push({ tipo: "texto", texto: "Respiración consciente. Deja que el cuerpo se asiente y pon una intención para la práctica.", min: 1 }); libres -= 1; }

      // en Desarrollo, la cumbre se elige ANTES que el resto: así las demás posturas de la
      // sección se escogen para prepararla (misma zona/dinámica), no al azar.
      var pk = null;
      if (s.peak) {
        pk = elegirCumbre(c.semilla, usados, nivelRank(s.nivelMax || "intermedio"));
        if (pk) usados[pk.slug] = 1;
      }
      // en Enfriamiento, si hubo cumbre, se busca contrapeso: misma zona (lo que trabajó),
      // pero la dinámica opuesta la pone la propia sección (dinMas ya pide flexión/torsión/quietud).
      var foco = pk ? { zona: pk.zona || [], dinamica: pk.dinamica || [] }
        : (cumbre && /enfriamiento/i.test(s.t)) ? { zona: cumbre.zona || [], dinamica: [] } : null;

      var topeN = s.maxN || tpl.maxN || 11;
      var n = CLAMP(Math.round(libres / (tpl.minPP || 1.4)), Math.min(2, topeN), topeN);
      if (pk) n = Math.max(1, n - 1); // un lugar ya lo ocupa la cumbre
      var elegidas = elegirPosturas(c.semilla, s, n, usados, foco);
      elegidas.forEach(function (p) { usados[p.slug] = 1; });

      // orden dentro de la sección: agrupa primero por familia de postura (de pie/sentada/…)
      // y, dentro de cada familia, por zona del cuerpo — así no salta de una zona a otra sin
      // relación. Desarrollo sube en dificultad hacia la cumbre; Enfriamiento va de activo a
      // pasivo (termina cerca del piso, justo antes de Savasana); el resto abre suave
      // (sentado/suelo) y cierra más activo, preparando el Desarrollo que sigue.
      if (s.peak) elegidas.sort(function (a, b) { return nivelRank(a.nivel) - nivelRank(b.nivel) || rangoFamilia(a) - rangoFamilia(b) || zonaRango(a) - zonaRango(b); });
      else if (/enfriamiento/i.test(s.t)) elegidas.sort(function (a, b) { return rangoFamilia(a) - rangoFamilia(b) || zonaRango(a) - zonaRango(b); });
      else elegidas.sort(function (a, b) { return rangoFamilia(b) - rangoFamilia(a) || zonaRango(a) - zonaRango(b); });

      // hold por postura: reparte los minutos libres, acotado al rango del estilo.
      // las bilaterales (×2) ocupan el doble, así que cuentan doble en el reparto.
      var pesoT = elegidas.reduce(function (a, p) { return a + (esBilateral(p) ? 2 : 1); }, 0) + (pk ? 1.6 : 0);
      var hold = CLAMP(Math.round(libres / Math.max(1, pesoT) * 2) / 2, rango[0], rango[1]);

      elegidas.forEach(function (p) {
        bl.items.push({ tipo: "postura", slug: p.slug, lado: esBilateral(p) ? "ambos" : "", dur: hold, durUnit: "min", nota: "" });
      });

      if (pk) {
        var at = CLAMP(Math.round(bl.items.length * 0.66), 1, bl.items.length);
        bl.items.splice(at, 0, { tipo: "postura", slug: pk.slug, lado: esBilateral(pk) ? "ambos" : "", dur: CLAMP(hold + 1, rango[0], rango[1] + 1), durUnit: "min", nota: "Postura cumbre" });
        cumbre = pk;
      }
      return bl;
    });
  ...
  function renderBuilder() {
    ...
    // armar la clase entera en un clic a partir de duración + estilo + semilla
    var autoWrap = el("div", "b-auto");
    var autoBtn = el("button", "b-auto-btn", "✨ Armar clase automática");
    autoBtn.type = "button";
    autoBtn.addEventListener("click", function () {
      var correr = function () { armarClaseAuto(c); renderBuilder(); flash("Clase armada — ajústala a tu gusto"); };
      if (nPosturas(c) > 1 || c.bloques.some(function (b) { return b.items.some(function (i) { return i.tipo === "texto" && (i.texto || "").trim(); }); })) {
        pedirConfirmacion("Esto reemplaza todas las secciones y posturas de ahora. ¿Armar de nuevo?", correr, "Armar");
      } else correr();
    });
    autoWrap.appendChild(autoBtn);
    autoWrap.appendChild(el("span", "b-hint", "Usa la duración, el estilo y la semilla de arriba. Te deja un borrador completo que puedes editar."));
