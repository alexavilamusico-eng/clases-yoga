...
(function () {
  ...
  /* ---------- armar clase automáticamente ----------
     Basado en cómo se estructura una clase de yoga profesional:
     arco de montaña — llegada → calentamiento → desarrollo con postura cumbre
     (~65-70% de la clase) → enfriamiento con contrapostura → Savasana.
     ...
     Fuentes: Yoga Journal (principios de secuenciación), método de postura
     cumbre (yogajala / yoga-sequencing), guías de Yin y Restaurativo. */
  ...
  function elegirPosturas(sem, s, n, usados) {
  ...
  function armarClaseAuto(c) {
    ...
    c.bloques = tpl.secs.map(function (s) {
      ...
      var elegidas = elegirPosturas(c.semilla, s, n, usados);
      elegidas.forEach(function (p) { usados[p.slug] = 1; });

      // orden: Desarrollo por dificultad ascendente; Enfriamiento del piso hacia arriba; el resto de suave a activo
      if (s.peak) elegidas.sort(function (a, b) { return nivelRank(a.nivel) - nivelRank(b.nivel) || rangoFamilia(a) - rangoFamilia(b); });
      else if (/enfriamiento/i.test(s.t)) elegidas.sort(function (a, b) { return rangoFamilia(b) - rangoFamilia(a); });
      else elegidas.sort(function (a, b) { return rangoFamilia(a) - rangoFamilia(b); });
      ...
      var pesoT = elegidas.reduce(function (a, p) { return a + (esBilateral(p) ? 2 : 1); }, 0) + (s.peak ? 1.6 : 0);
      ...
      if (s.peak) {
        var pk = elegirCumbre(c.semilla, usados, nivelRank(s.nivelMax || "intermedio"));
        if (pk) {
          usados[pk.slug] = 1;
